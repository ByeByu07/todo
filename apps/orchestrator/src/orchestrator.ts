...
export class Orchestrator {
  ...
  private async dispatch(issue: any): Promise<void> {
    ...
    try {
      ...
      // Post "starting" comment on first run only
      if (!meta?.hasStarted) {
        this.state.trackIssueMeta(issue.id, { hasStarted: true });
        const startBody = this.buildStartingComment(issue);
        this.githubClient!.postComment(issue.number, startBody);
      }
  ...
  private buildStartingComment(issue: any): string {
    let body = "## \ud83e\udd16 Symphony Agent \u2014 Starting Work\n\n";
    body += `**Task:** ${issue.title}\n\n`;
    ...
    const branch = `symphony/${issue.number}-${issue.title.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "")}`;
    body += `**Branch:** \`${branch}\`\n\n`;
    body += "I'll update this thread when I have a PR ready for review.";
  ...
  private async postAgentComment(
    ...
    body += `\n### \ud83d\udd04 Revision Todo\n`;
    body += `- [ ] Human review the PR\n`;
    body += `- [ ] If changes needed: comment \`/retry\` with specific feedback\n`;
    body += `- [ ] Agent will re-run and update the existing PR\n`;
    body += `- [ ] If approved: merge PR to auto-close this issue\n`;
